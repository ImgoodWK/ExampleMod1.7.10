...
public abstract class LZGT_MultiMachineBase<T extends LZGT_MultiMachineBase<T>> extends GT_MetaTileEntity_ExtendedPowerMultiBlockBase<T> implements IConstructable, ISurvivalConstructable {
    public LZGT_MultiMachineBase(int aID, String aName, String aNameRegional) {
        super(aID, aName, aNameRegional);
    }

    public LZGT_MultiMachineBase(String aName) {
        super(aName);
    }
    ...
    protected ProcessingLogic createProcessingLogic() {
        return (ProcessingLogic)(new LZGT_ProcessingLogic() {
            public @NotNull CheckRecipeResult process() {
                this.setEuModifier(LZGT_MultiMachineBase.this.getEuModifier());
                this.setSpeedBonus(LZGT_MultiMachineBase.this.getSpeedBonus());
                this.setOverclock(LZGT_MultiMachineBase.this.isEnablePerfectOverclock() ? 2 : 1, 2);
    ...
    public String[] getInfoData() {
        ...
        ret[origin.length] = EnumChatFormatting.AQUA + LZGTTextHandler.texter("Parallels", "MachineInfoData.Parallels") + ": " + EnumChatFormatting.GOLD + this.getLimitedMaxParallel();
        ret[origin.length + 1] = EnumChatFormatting.AQUA + LZGTTextHandler.texter("Speed multiplier", "MachineInfoData.SpeedMultiplier") + ": " + EnumChatFormatting.GOLD + dSpeed;
        ret[origin.length + 2] = EnumChatFormatting.AQUA + LZGTTextHandler.texter("EU Modifier", "MachineInfoData.EuModifier") + ": " + EnumChatFormatting.GOLD + dEUMod;
        return ret;
    }
    ...
    public boolean addEnergyOutputMultipleDynamos(long aEU, boolean aAllowMixedVoltageDynamos) {
        ...
        long leftToInject;
        long aVoltage;
        for(Iterator var10 = ((ArrayList)Utils.filterValidMTEs(this.mDynamoHatches)).iterator(); var10.hasNext(); totalOutput += aVoltage) {
